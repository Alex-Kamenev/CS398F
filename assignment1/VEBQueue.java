...
public class VEBQueue<T> {
    ...
    private T min_data;
    private T max_data;
    private VEBQueue summary;
    private VEBQueue[] cluster;

    //helper method for higher sqrt of u
    private int highRoot (int universe){
        int result = (int) Math.pow(2.0 , Math.ceil((Math.log(universe)/Math.log(2.0)) / 2.0));
        return result;
    }

    //helper method for lower sqrt of u
    private int lowRoot (int universe){
        int result = (int) Math.pow(2.0 , Math.floor((Math.log(universe)/Math.log(2.0)) / 2.0));
        return result;
    }
    ...
    public VEBQueue(int size) {
        ...
        //if u > 2
            ...
            for (int i = 0; i < uReduced; i++) {
                cluster[i] = new VEBQueue((int) lowRoot(u));
            }
        }
    }
    ...
    public void changePriority(VEBQueue vEB, int oldKey, int newKey){
        if(!(vEB.vEBMember(vEB, newKey)) && (newKey > -1 && newKey < vEB.u)) {
            T data = (T) vEB.getData(vEB, oldKey);
    ...
    public T getData(VEBQueue vEB, int x){
        if(x == vEB.min){
    ...
    private void baseInsert(VEBQueue vEB, int x, T data){
        vEB.max = x;
        vEB.min = x;
        vEB.max_data = data;
        vEB.min_data = data;
...
//                    vEB.cluster[vEB.high(x)].max = vEB.low(x);
    ...
    public static void main(String[] args) {
        VEBQueue newQueue = new VEBQueue((int) Math.pow(2, (2*3+1)));
